...
<!-- widget grid -->
<section id="member-widget" class="" style="background-image:url(img/r2.jpg);opacity:1;width:100%;margin-left:-10px;z-index:-50px;">
	<div class="row" >
...
<script type="text/javascript">
	 ...
	 */
	 ...
	 $(function() {
		...
		// 设置gird
		grid.jqGrid({
			url: 'emp/listBusiness',
			caption: '<span class="glyphicon glyphicon-list"></span> 商家信息列表',
			colModel: [
		...
		// detail-grid
		detailGrid.jqGrid({			
			caption: '<span class="glyphicon glyphicon-list"></span> 所属商家：无',
			colModel: [
				{label: 'ID', name: 'id', hidden: true},
				{label: '图书名称', name: 'name', width: 100},
				{label: '图书类型', name: 'bookType', width: 100},
				{label: '价格', name: 'price', width: 100},
				{label: '库存', name: 'stock', width: 100},
			],
			pager: '#detail-pager'
		});
		// 设置导航条
		...
		function addfunc() {
			...
			if(!id) {
				layer.msg('请先选择左侧的会员！', {icon: 5});
				return;
			}
			top.layer.open({
				type: 2,
				title: '图书选择器',
				content: '<%=path%>/pages/selector/goods_selector.jsp',
				area: ['420px', '620px'],
				maxmin: true,
				btn: ['确定', '取消'],
				yes: function(index, layero) {
					// 调用iframe页面中的函数
					var arr = layero.find('iframe')[0].contentWindow.getSelectGoods();
					if(arr != null) {
						//arr[0]书籍类型的id, id 会员id
						$.post('./emp/save/'+arr[0]+'/'+id,  function(json){
				    		if(json.success) {
		...
		function delfunc(rowid) {
			layer.confirm('确定移除该书籍吗？', {icon:3, title:'温馨提示'}, function(index) {
				$.getJSON('./emp/delGoods',{ids: rowid+''}, function(json) {
					if(json.success) {
						detailGrid.trigger("reloadGrid", [{page: 1}]);
		...
		function showDetail(rowId, selected) {
			if(rowId != null) {
				...
				detailGrid.jqGrid('setGridParam', {url: 'goods/get/'+rowId});
				detailGrid.jqGrid('setCaption', '<span class="glyphicon glyphicon-list"></span> 所4·：'+row.name);
				detailGrid.trigger("reloadGrid");
			}						
		};
		// 清除detail
		function clearDetail() {
			detailGrid.jqGrid('setGridParam',{url: 'emp/list/-1'}); //todo: 
			detailGrid.jqGrid('setCaption', '<span class="glyphicon glyphicon-list"></span> 所属会员：无');
			detailGrid.trigger("reloadGrid");
		}
