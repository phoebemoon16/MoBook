...
<section id="instSelector" class="container">
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
			<table>
				<tr>
					<td>
						所属学院 <input type="text" style="width:120px;height:30px;" name="Q_LKS_iname">
	...
	<!-- end row -->
		...
		$(function() {
			grid.jqGrid({
				...
				colModel: [
					...
					{label:'iname', name: 'iname', hidden: true},
					{label: '所属学院', name: 'note', width: 100, sortable:false, formatter: showIname}
				],
				hidegrid: false,
				...
				gridComplete: function() {
					...
					if(params.selectInstIds) {
						setSelectInst(params.selectInstIds);
					}
				}
			});
			// 给treeGrid增加多选框
			function showIname(v, options, row) {
			    var rowId = row.id;
			    var iname = row.iname;
			    var checkbox = '<input type="checkbox" name="cb" id="chx_'+rowId+'" class="ace" value="'+rowId+'" onclick="clickCheckbox('+rowId+', this);" />'+
	                     	   '<span class="lbl align-top" ></span> '+ iname;
			    return checkbox ; 
			}
		...
		// 获得选中值, 是否显示非叶子结点
		function getSelectInst(isNotLeaf) {
			var ids = [];
			$("input[name=cb]:checked").each(function() {
				ids.push($(this).val());
			});
			
			var instIds=[], instNames=[];
			...
			var retval = [];
			retval[0] = instIds.join(',');
			retval[1] = instNames.join(',');
			return retval;
		}
		
		// 初始化
		function setSelectInst(ids) {
			var arr = ids.split(',');
